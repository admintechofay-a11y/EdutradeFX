...
export default function SignalProviderDetailPage({ params }: { params: { id: string } }) {
  const provider = MOCK_SIGNAL_PROVIDERS.find((p) => p._id === params.id) || MOCK_SIGNAL_PROVIDERS[0];
  ...
  if (!provider) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center">
        <h2 className="text-2xl font-bold text-white">Signal Provider Not Found</h2>
        <p className="text-sm text-slate-400 mt-2">The requested signal provider does not exist.</p>
        <Link
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 pb-8 border-b border-slate-800">
          <div className="flex items-start sm:items-center gap-5">
            <div className="relative">
              <img
                ...
                alt={provider.name}
                className="w-20 h-20 sm:w-24 sm:h-24 rounded-3xl object-cover border-2 border-amber-500/50 shadow-glow-gold"
              />
              <div
                className="absolute -bottom-1.5 -right-1.5 bg-amber-500 text-brand-darkest rounded-full p-1 shadow-md"
                title="Audited Signal Provider"
              >
            ...
            <div className="space-y-1.5">
              <div className="flex flex-wrap items-center gap-2">
                <span className="badge-regulation text-xs">Live Signals</span>
                {provider.isFeatured && <span className="badge-gold text-xs">Top Rated</span>}
                <span className="badge-green text-xs">${provider.subscriptionPrice}/month</span>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 pt-1">
                {provider.markets.map((m) => (
                  <span key={m} className="px-2 py-0.5 rounded-md bg-brand-surface border border-slate-700 text-[11px] font-semibold text-slate-300">
                    {m}
                  </span>
                ))}
              </div>
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            ...
            <a
              ...
              className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-emerald-500 text-brand-darkest font-bold text-xs flex items-center gap-1.5 shadow-glow-gold hover:opacity-90 transition-all"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 pt-8">
          ...
          <div className="bg-brand-surface/70 rounded-2xl p-4 border border-slate-800 text-center">
            ...
            <span className="text-xl font-black text-cyan-400 mt-1 block">
              +{provider.historicalPerformance.totalPips.toLocaleString()}
            </span>
          </div>
          ...
          <div className="bg-brand-surface/70 rounded-2xl p-4 border border-slate-800 text-center">
            ...
            <span className="text-xl font-black text-purple-400 mt-1 block">
              {provider.historicalPerformance.profitFactor}
            </span>
          </div>
          ...
          <div className="bg-brand-surface/70 rounded-2xl p-4 border border-slate-800 text-center">
            ...
            <span className="text-xl font-black text-white mt-1 block">
              ~{provider.historicalPerformance.monthlySignals}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="rounded-2xl glass-card border border-slate-800 p-6 space-y-4">
            <h2 className="text-lg font-bold text-white">Signal Methodology & Delivery</h2>
            <p className="text-sm text-slate-300 leading-relaxed whitespace-pre-line">
              {provider.description}
            </p>
            <div className="rounded-xl bg-brand-surface/80 border border-slate-800 p-4 mt-4">
              <h3 className="text-xs font-bold text-amber-400 uppercase tracking-wider">
                Analysis & Confirmation Rules
              </h3>
              <p className="text-xs text-slate-300 mt-1 leading-relaxed">{provider.strategy}</p>
            </div>
          </div>
          ...
          <div className="rounded-2xl glass-card border border-slate-800 p-6 space-y-4">
            ...
            <p className="text-sm text-slate-300 leading-relaxed">{provider.riskInfo}</p>
            <div className="rounded-xl bg-amber-500/10 border border-amber-500/30 p-4">
              <div className="flex items-start gap-3">
                <AlertTriangle className="w-5 h-5 text-amber-400 shrink-0 mt-0.5" />
                <p className="text-xs text-slate-300 leading-relaxed">{provider.disclaimer}</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">
                  Feedback from active subscribers copying this feed
                </p>
              </div>
            </div>
              ...
              {isAuthenticated ? (
                <form onSubmit={handleReviewSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Accurate TP1 targets and clear reasoning"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      placeholder="Share your experience with trade execution, Telegram delivery speed, and overall accuracy..."
                ...
                <div className="rounded-xl bg-brand-surface/80 border border-slate-800 p-5 text-center space-y-2">
                  ...
                  <p className="text-xs text-slate-300 font-medium">
                    You must be logged in to submit a verified signal review.
                  </p>
                  <Link
                    href="/login"
          ...
          <div className="rounded-2xl glass-card border border-slate-800 p-6 space-y-4">
            ...
            <div className="space-y-3 text-xs">
              <div className="flex justify-between py-2 border-b border-slate-800">
                ...
                <span className="font-bold text-white">${provider.subscriptionPrice}/mo</span>
              </div>
              <div className="flex justify-between py-2 border-b border-slate-800">
                <span className="text-slate-400">Delivery Channels</span>
                <span className="font-bold text-amber-400">Telegram & Webhook</span>
              </div>
              <div className="flex justify-between py-2 border-b border-slate-800">
                <span className="text-slate-400">Primary Pairs</span>
                <span className="font-bold text-white">{provider.markets.join(', ')}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-slate-800">
                <span className="text-slate-400">Verification</span>
                <span className="font-bold text-emerald-400">Audited Feed</span>
              </div>
            </div>

            <a
              href={`mailto:${provider.contactEmail}`}
              className="w-full mt-4 py-3 rounded-xl bg-amber-500 hover:bg-amber-400 text-brand-darkest font-extrabold text-xs flex items-center justify-center gap-1.5 transition-all shadow-glow-gold"
            >
              <Send className="w-4 h-4" />
              Subscribe Now
            </a>
